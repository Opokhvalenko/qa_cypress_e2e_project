...
import { router, store } from '../../public/js/_app.js';
...
export default {
      ...
      axios
        ...
        .then((response) => {
          if (response.data.success === true) {
            const articles = context.getters.articles;
  ...
  deleteArticleTag(context, tag) {
    ...
    const article = context.getters.article;
  ...
  setFollowProfile(context, params) {
    console.log('Handling action: setFollowProfile');
    const action = params.value === false ? 'unfollow' : 'follow';
    return new Promise((resolve) => {
      axios
        .post(`/profiles/${params.username}`, {
          action,
          user_id: context.getters.user.id
        })
        .then((response) => {
        ...
        .catch((error) => {
          console.log('Follow/unfollow unsuccessful.');
          console.log(error.response);
          resolve(error.response);
        });
    });
